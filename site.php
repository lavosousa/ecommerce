...
$app->get("/cart/:idproduct/add", function($idproduct) {
	...
	$cart = Cart::getFromSession();  // Pega o carrinho da sess√£o ou cria um novo

	$qtd = (isset($_GET['qtd'])) ? (int)$_GET['qtd'] : 1 ;  // Se no parametro de quantidades de produtos veio mais de um... se n√£o fica 1.
...
$app->get("/checkout", function() {
	...
	User::verifyLogin(false);   // para uma rota que nao √© para admin

	$address = new Address();
	$cart = Cart::getFromSession();   // ja pega o carrinho da sess√£o
	...
	if (isset($_GET['zipcode'])) {
		...
		$cart->setdeszipcode($_GET['zipcode']);  // ja pega o carrinho da sess√£o
...
$app->post("/checkout", function(){
	...
	if (!isset($_POST['desaddress']) || $_POST['desaddress'] === '' ) {
		Address::setMsgError("Informe o endere√ßo");
		header('Location: /checkout');
		exit;
	}
	...
	$totals = $cart->getCalculateTotal();     //debug:Retornou vazio
...
$app->post('/register', function() {
	...
	if (User::checkLoginExists($_POST['email']) == true ) {
		...
		User::setErrorRegister("Este endere√ßo de e-mail j√° est√° sendo usado por outro usu√°rio.");
		header("Location: /login");
		exit;
...
$app->post('/forgot/reset', function() {
	...
	User::setForgotUsed($forgot["idrecovery"]); // PAra invalidar o c√≥digo apos uso.
...
$app->post("/profile", function (){
	...
	if ($_POST['desemail'] !== $user->getdesemail()) {   // p/ ver se mudou email.
		...
		if (User::checkLoginExists($_POST['desemail']) === true ) {
			...
			User::setError("Este endere√ßo de email j√° esta cadastrado.");
			header('Location: /profile');
			exit;
	...
	// Sobrecarrgar o $_POST para que o inadmin/senha n√£o seja sobrescritos pelo que vem do post.
	$_POST['inadmin']  = $user->getinadmin();
	$_POST['despassword'] = $user->getdespassword();
	$_POST['deslogin'] = $_POST['desemail'];   // j√° seta o login como o e-mail.
...
$app->get("/order/:idorder", function($idorder) {
    ...
    $order->get((int)$idorder);  // Carregar o pedido
...
$app->get("/boleto/:idorder", function($idorder){
    ...
    $dadosboleto["carteira"] = "175";  // CÛdigo da Carteira: pode ser 175, 174, 104, 109, 178, ou 157
...
$app->post("/profile/change-password", function() {
    ...
    if ($_POST['new_pass'] !== $_POST['new_pass_confirm']) {                
        User::setError("A nova senha e sua confirmacao devem ser iguais.");
        header("Location: /profile/change-password");
        exit;        
    }
    ...
    if (!password_verify($_POST['current_pass'], $user->getdespassword())) {        
        User::setError("A senha atual digita nao corresponde a senha atual da conta");
        header("Location: /profile/change-password");
        exit;        
    }
